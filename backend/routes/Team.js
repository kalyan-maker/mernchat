...
router.get("/my", verifyToken, (req, res) => {
  Team.find({ user: req.user.id }, { __v: 0, createdAt: 0 })
    .populate("user", ["username"])
    .then((teams) => {
      return res.status(200).json({
        status: true,
...
router.post(
  ...
  [
    check("name")
      .not()
      .isEmpty()
      .withMessage("Please enter team name.")
      .trim()
      .escape(),
    check("description")
      .not()
...
router.post("/getTeam/:teamID", (req, res) => {
  const teamID = req.params.teamID;

  Team.findById(teamID, { createdAt: 0, __v: 0 })
    .populate("user", ["username"])
    .then((team) => {
      if (team) {
        return res.status(200).json({
