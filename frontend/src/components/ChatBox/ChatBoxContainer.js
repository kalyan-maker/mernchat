...
class ChatBoxContainer extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .post(`${apiBaseURL}/api/team/getTeam/${query.get("tid")}`)
      .then((res) => {
  ...
  handleSubmit = (e) => {
    ...
    axios
      .post(`${apiBaseURL}/api/chat/new`, chatMessage)
      .then((res) => {
  ...
  handleUserClick = (e, id) => {
    ...
    if (this.state.oldClID === "") {
      ...
      document
        .getElementById(this.state.oldClID)
  ...
  getChatMessages = () => {
    ...
    axios
      .post(`${apiBaseURL}/api/chat/getTeamUser/`, {
        team: query.get("tid"),
      })
      .then((res) => {
        ...
        for (index = 0; index < users.length; index++) {
          if (!this.state.user_id.has(users[index])) {
            // send axios AJAX request to get team data
            axios
              .post(`${apiBaseURL}/api/user/getUser/${users[index]}`)
              .then((res) => {
    ...
    axios
      .post(`${apiBaseURL}/api/chat/getMessages/${query.get("tid")}`)
      .then((res) => {
